...
if versioninfo
	md"""
- **0.0.1**: not ready to release

"""	
end

# ╔═╡ f3aa400c-9800-11ee-2d39-417338333971
md"""# Brown-Driver-Briggs lexicon"""

# ╔═╡ 45b1f006-8017-4d8d-b8a0-ac0781419900
md"""## Search lemma"""

# ╔═╡ 349668f0-0934-4fdf-91db-dbfd8356b684
md"""### Search article text"""

# ╔═╡ 25104214-ce36-413a-972d-1373d6332d61
md"""*Search for* $(@bind fulltext confirm(TextField(default = "necromancer")))"""
...
bdbfile = joinpath(pwd() |> dirname, "src", "lex-md.cex")
...
# ╔═╡ 5dac6956-c10c-43dd-96b7-6fae0bcb3373
textmatches = filter(artcl -> occursin(fulltext, artcl.article), bdbarticles)

# ╔═╡ 075b458d-33f2-4162-8700-6fdbc41f69cf
isempty(textmatches) ? md"No matching articles." : md"""Matching articles ($(length(textmatches)) total): """
...
# ╔═╡ 51939301-2df2-44d7-bc2a-663f6a5b57a2
xcriptdict = Dict([
		'a' => 'א',
		'b' => 'ב',
		'g' => 'ג',
...
# ╔═╡ c8590586-6fef-4f71-a25f-1e05ec3e9bdf
"""Look up a lemma in a given format in BDB."""
function lookup(lemma, format, bdb)
	matches = []
	if format == :fullucode
		matches = filter(artcl -> artcl.lemma == lemma, bdb)
	elseif format == :consonantucode
		matches = filter(artcl -> BiblicalHebrew.unpointed(artcl.lemma) == lemma, bdb)
	else # transcription
		xcribed = map(c -> xcriptdict[c], lowercase(lemma))
		matches = filter(artcl -> BiblicalHebrew.unpointed(artcl.lemma) == xcribed, bdb)
	end
	matches
end

# ╔═╡ 596666c3-9e13-4d7c-834d-608fd16a5935
...
# ╔═╡ 5043ee9d-bd0d-416f-b79a-1628a07d0fb4
 md"""*Format* $(@bind lemmaformat Select(formatmenu, default = :xcript))"""

# ╔═╡ 99d3df6a-d19c-4ceb-af68-3d5c37d40ada
if lemmaformat == :xcript
aside(md"""
| Hebrew | ASCII |
| --- | --- |
| א | a |
...
# ╔═╡ 6efdcf86-17b5-4906-b736-5443199a1466
"""Format a BDB article object for markdown display."""
function formatarticle(bdbarticle)
	 string("ּּ`", bdbarticle.id, "` ", " **", bdbarticle.lemma, "**. ", replace(bdbarticle.article, "¶" => " **¶**  "))
end

# ╔═╡ 4dbd6f74-13a3-4a3e-8a48-e71bb9c61948
begin
	textmatchesmd = map(atcl -> formatarticle(atcl), textmatches)
	isempty(textmatchesmd) ? md""  :	"1. " * join(textmatchesmd, "\n\n1. ")|> Markdown.parse
end

# ╔═╡ e1720484-fea9-489b-ba9c-ac66ccc13be5
"""Compose default lemma value in correct format."""
function defaultlemma()
	if lemmaformat == :xcript
		"qyl"
	elseif lemmaformat == :fullucode
		"קָטַל"
	elseif lemmaformat == :consonantucode
		"קטל"
	end
end

# ╔═╡ baeea2c6-d605-471c-8dc4-609aba837d1f
md"""*Search for* $(@bind lemmatext confirm(TextField(default=defaultlemma()))) """

# ╔═╡ 51d2e276-2fab-49c2-91bf-e5c1bfae5190
begin
	if lemmaformat == :xcript
		xcribed = map(c -> xcriptdict[c], lowercase(lemmatext))
		md"*Searched for* $(xcribed)"
	else
		md"*Searched for* $(lemmatext)"
	end
end

# ╔═╡ 885d5f54-60e3-49a2-b17e-8905028b3b2c
matchingarticles = lookup(lemmatext, lemmaformat, bdbarticles)

# ╔═╡ ce9aaec6-fcb9-4ed3-8f08-d1e0656ad108
isempty(matchingarticles) ? md"No matching articles." : md"""Matching articles:"""

# ╔═╡ f841ff98-8d52-4cf2-bb8f-185f0039e6bf
begin
	mdarticles = map(atcl -> formatarticle(atcl), matchingarticles)
	isempty(mdarticles) ? md""  :	display = "1. " * join(mdarticles, "\n\n1. ")|> Markdown.parse
end

# ╔═╡ d9599c5e-e2e5-49f2-b8eb-60a04586cbd5
md"""The following (hidden) cell hijacks Pluto CSS to get fonts including pointed Hebrew."""
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "009f080a4b8a40050af283222ccddbdbb9ccaa79"
...
git-tree-sha1 = "3fed303e0c79e223f48ea3a6f2da5b1424a0e2ad"
